...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_devices(self, user_input=None):
        ...
        schema = {vol.Optional(f"{d.get('name', m)} ({d.get('product_model', '')})", default=False): bool for m, d in sorted(states.items())}
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            for key in [CONF_PASSWORD, CONF_KEY_ID, CONF_API_KEY]:
                if val := user_input.get(key): new_data[key] = val.strip()
